...
export default function ObdLivePanel({ setSuccessToast, onInjectSymptom }: Props) {
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {ids.map((id) => {
          ...
          const hot =
            id === 'coolant' && sample?.value != null && sample.value >= 105;
          return (
            <div
              key={id}
              className={`rounded-xl border p-2.5 ${
                hot
                  ...
                  : 'border-tech-borda bg-tech-fundo/60'
              }`}
            >
      ...
      {open && (
        <div className="px-4 pb-4 space-y-4 border-t border-tech-borda/60 pt-3">
          <p className="text-[11px] text-slate-400 leading-relaxed">
            Conecte um <strong className="text-slate-200">ELM327 BLE</strong> (Bluetooth Low Energy) ou use o
            simulador. Adaptadores clássicos SPP exigem app nativo (Capacitor).
          </p>
